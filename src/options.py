...
class Options():
    ...
    def add_optim_options(self):
        ...
        self.parser.add_argument('--early_stop', default=30, type=int)
        self.parser.add_argument('--dataset', default='okvqa', type=str)
        self.parser.add_argument('--stream', default=1, type=int)
        self.parser.add_argument('--use_fact', default='yes', type=str)
        self.parser.add_argument('--fact_use_way', default="concate", type=str)
        # mean / max / 21mean /
        self.parser.add_argument('--attention_score_style', default="mean", type=str)
        self.parser.add_argument('--consider_context_attention', default="no", type=str)
        self.parser.add_argument('--use_last_half_layer_attention', default="no", type=str)
        self.parser.add_argument('--ans_attention', default="no", type=str)
        self.parser.add_argument('--version', default="v1", type=str)

        self.parser.add_argument('--asymmetric_retri', default="no", type=str)
    ...
    def add_retriever_options(self):
        self.parser.add_argument('--indexing_batch_size', type=int, default=50000, help="Batch size of the number of passages indexed")
        self.parser.add_argument("--n-subquantizers", type=int, default=0, help='Number of subquantizer used for vector quantization, if 0 flat index is used')
        self.parser.add_argument("--n-bits", type=int, default=8, help='Number of bits per subquantizer')
        self.parser.add_argument('--passages_embeddings', type=str, default='fact_embedding_dim256_at_11-07-13.pkl', help='Glob path to encoded passages')
        self.parser.add_argument("--save_or_load_index", action='store_true', help='If enabled, save index and load index if it exists')
        self.parser.add_argument('--train_data', type=str, default='none', help='path of train data')
        self.parser.add_argument('--eval_data', type=str, default='none', help='path of eval data')
        self.parser.add_argument('--indexing_dimension', type=int, default=256)
        ...
        self.parser.add_argument('--question_maxlength', type=int, default=130,
                                 help='maximum number of tokens in questions')
        self.parser.add_argument('--passage_maxlength', type=int, default=130,
